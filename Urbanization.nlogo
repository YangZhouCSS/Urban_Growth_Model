  ...
  while [i > -1]
    [ set the-row []
      set the-row patches with [pycor = i] 
        foreach sort-on [pxcor] the-row [ask ? [ifelse urban = 1 [file-write 1] [file-write 0]]]
     file-print "   \r\n"
     set i i - 1] 
...
1235
366
This comment will export the current map into the result.asc file in the data folder.
11
0.0
1
...
![NetLogo](http://s16.postimg.org/4ic0wmb4z/results.jpg)

The image below shows the road types and landuse types in the area studied.

![NetLogo](http://s15.postimg.org/dhwbgxl55/results2.jpg)
